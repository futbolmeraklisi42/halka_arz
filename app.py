...
ESKI_HALKA_ARZ_VERISI = [
    ...
    {"kod": "ZERGY", "ad": "Zer GYO", "maliyet": 13.00, "satis_fiyati": 19.22, "lot": 193 * 3, "sahip": "Ortak (3 Hesap)", "hesap_sayisi": 3},
    {"kod": "ARFYE", "ad": "Arf Bio Enerji", "maliyet": 19.50, "satis_fiyati": 31.34, "lot": 47 * 3, "sahip": "Ortak (3 Hesap)", "hesap_sayisi": 3},
    {"kod": "MEYSU", "ad": "Meysu Gıda", "maliyet": 7.50, "satis_fiyati": 15.96, "lot": 128 * 3, "sahip": "Ortak (3 Hesap)", "hesap_sayisi": 3},
    {"kod": "FRMPL", "ad": "Formplast", "maliyet": 30.24, "satis_fiyati": 51.24, "lot": 40 * 3, "sahip": "Ortak (3 Hesap)", "hesap_sayisi": 3},
    {"kod": "ZGYO", "ad": "Ziraat GYO", "maliyet": 9.77, "satis_fiyati": 20.98, "lot": 111 * 3, "sahip": "Ortak (3 Hesap)", "hesap_sayisi": 3},
    {"kod": "UCAYM", "ad": "Üçler Madencilik", "maliyet": 18.00, "satis_fiyati": 35.00, "lot": 54 * 3, "sahip": "Ortak (3 Hesap)", "hesap_sayisi": 3},
    {"kod": "AKHAN", "ad": "Akhan Un", "maliyet": 21.50, "satis_fiyati": 38.46, "lot": 35 * 3, "sahip": "Ortak (3 Hesap)", "hesap_sayisi": 3},
    {"kod": "NETCD", "ad": "Netcad Yazılım", "maliyet": 46.00, "satis_fiyati": 99.50, "lot": 25 * 3, "sahip": "Ortak (3 Hesap)", "hesap_sayisi": 3},
    ...
    {"kod": "SVGYO", "ad": "Sevil GYO", "maliyet": 3.64, "satis_fiyati": 9.43, "lot": 174 * 4, "sahip": "Ortak (4 Hesap)", "hesap_sayisi": 4},
    {"kod": "LXGYO", "ad": "Lux GYO", "maliyet": 12.05, "satis_fiyati": 26.94, "lot": 55 * 4, "sahip": "Ortak (4 Hesap)", "hesap_sayisi": 4},
    ...
    {"kod": "GOLDA", "ad": "GOLDA MAKARNA", "maliyet": 9.20, "satis_fiyati": 16.30, "lot": 130 * 4, "sahip": "Ortak (5 Hesap)", "hesap_sayisi": 5},
    {"kod": "ISVEA", "ad": "ISVEA", "maliyet": 20.90, "satis_fiyati": 26.50, "lot": 47 * 4, "sahip": "Ortak (5 Hesap)", "hesap_sayisi": 5},
    {"kod": "SSAAT", "ad": "SAAT VE SAAT", "maliyet": 56.00, "satis_fiyati": 52.70, "lot": 72 * 4, "sahip": "Ortak (5 Hesap)", "hesap_sayisi": 5},
...
with tab1:
    ...
    with sub_tab2:
        ...
        if df_satilan.empty:
            ...
            for kod in unique_satilan_kods:
                sub_df_sat = df_satilan[df_satilan["kod"] == kod]
                sirket_adi = sub_df_sat.iloc[0]["ad"]
                ...
                for _, r in sub_df_sat.iterrows():
                    l = safe_float(r['lot'], 1)
                    mal = safe_float(r['maliyet'])
                    sat = safe_float(r['satis_fiyati'])
                    toplam_maliyet_sat += l * mal
                    toplam_gelir_sat += l * sat
                
                toplam_net_kar_sat = toplam_gelir_sat - toplam_maliyet_sat
                ort_maliyet = toplam_maliyet_sat / toplam_lot_sat if toplam_lot_sat > 0 else 0
                ort_satis = toplam_gelir_sat / toplam_lot_sat if toplam_lot_sat > 0 else 0
                genel_kar_orani = ((ort_satis - ort_maliyet) / ort_maliyet) * 100 if ort_maliyet > 0 else 0

                kar_orani_str = f"+%{genel_kar_orani:.2f}" if genel_kar_orani >= 0 else f"-%{abs(genel_kar_orani):.2f}"
                net_kar_str = f"+₺{toplam_net_kar_sat:,.2f}" if toplam_net_kar_sat >= 0 else f"-₺{abs(toplam_net_kar_sat):,.2f}"
                badge_class_sat = "badge-kar-yesil" if toplam_net_kar_sat >= 0 else "badge-zarar-kirmizi"
                ...
                with st.container(border=True):
                    ...
                    with c2:
                        st.markdown("📦 *Toplam Lot*")
                        st.markdown(f"*{toplam_lot_sat} Lot*")
                    with c3:
                    ...
                    with st.expander("👥 Kişi/Hesap Satış Detaylarını Göster & Sil", expanded=False):
                        ...
                        for idx, row in sub_df_sat.iterrows():
                            ...
                            with kc1:
                                ...
                                st.caption(f"📦 {lot} Lot")
                            with kc2:
                                st.write(f"Alış: ₺{maliyet:.2f} ➔ Satış: *₺{satis_fiyati:.2f}*")
                            with kc3:
                                st.markdown(f"Kâr: <span style='color:{k_kar_renk}; font-weight:bold;'>{k_kar_str}</span>", unsafe_allow_html=True)
                            with kc4:
